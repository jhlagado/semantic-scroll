...
Magnetic tape deteriorates, and the recording would preserve damage along with the data. I had to establish how Commodore encoded the program, then measure the pulse timings and extract bytes. Interpreting those bytes as BASIC, the language we'd used, produced readable descriptions and instructions, although some early output contained dots and unfamiliar symbols. I initially aimed for an uppercase transcription. Checking Commodore's character encoding showed that some apparent damage came from my conversion; the original letter case and screen controls were still present.
...
Reading the code also helped me examine its history. Hyperdrive has a compass, an odd accessory aboard a spacecraft, inherited from Caverns. I could follow that connection to establish what Ken had retained or changed, comparing the code with what I remembered. The cassette preserved the program; I had to construct an account of its history from that evidence and the fragments I still recalled.
