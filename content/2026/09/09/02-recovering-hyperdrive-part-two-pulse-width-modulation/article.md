...
There was the game's name, followed by Micro Parts, Ken's business, and the date. Room descriptions and prompts appeared elsewhere among fragments of instructions. After looking through so many numbers, I was finally reading Ken's words. But I couldn't yet treat everything printable as text. The stray K in `KHYPERDRIVE` came from a number immediately before the filename, whose value happened to match the character code for K. My extraction joined them together. To get a reliable listing, I needed the structure around those words too.
...
The first converter inserted dots for values it didn't translate. Those nine dots covered a black-text control, five moves to the right, two moves down and an uppercase H. Ken selected mixed-case mode for the title, so the remaining letters would appear in lowercase: *Hyperdrive*. The second conversion recovered the H, but still replaced cursor movements with spaces and printed the word in capitals. It looked cleaner, yet I was still losing part of what Ken had put there.

Commodore even let you put screen operations inside quotation marks along with the text. A colour-key press could insert a control byte into a string; printing that string would change the colour of the letters that followed. Cursor movements worked in much the same way. During a BASIC listing on the VIC, those controls could appear as reversed symbols that you could edit alongside the words. This gave Ken a way to arrange his title and colour his text within a `PRINT` statement. Decades later, I was replacing those instructions with dots. Even the value 144 depended on context: it meant the BASIC keyword `STOP` in an instruction, but selected black text inside an output string. I had to follow the quotation marks and interpret the bytes accordingly to recover what Ken had put on the screen.

By this stage I could read the descriptions and trace the logic of a game I couldn't have recreated from memory. Ken had fitted it into roughly fourteen kilobytes, and working through his words and choices brought back details I hadn't thought about for decades. The exact spelling of a notice, the case of a letter and the positioning of a title were all still there to recover. I'd started out willing to lose the letter case and colour for the sake of reading anything at all. Now I wanted to see the title appear as he'd arranged it, enter a command and explore the game again.
