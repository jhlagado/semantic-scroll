...
Scheme lets a programmer build data as a calculation unfolds. A list can be assembled, passed to another procedure and left behind when it is no longer needed, with no matching instruction to free each pair. The code describes the calculation rather than the lifetime of every allocation. That freedom moves the storage work into the runtime of the compiled program. In Skate, my Scheme compiler for Z80/CP/M, that runtime shares the machine's 64 KiB address space with the program and its data.

That list consists of pairs, small two-slot records. One part of each pair holds an item and the other refers to the next pair. A reference to the first pair is enough to find the entire list by following those links. When the last reference to a list disappears, its pairs still occupy memory until the runtime recovers them. It must distinguish those pairs from ones the program can still use.
