...
Scheme is appealing because it lets a programmer express an algorithm through procedures and the values flowing between them, rather than spelling it out chiefly as a sequence of changes to storage. That style has its roots in lambda calculus and often feels closer to how we reason through a problem. Lists and other structures can take shape as the calculation proceeds and remain available for as long as the program can use them. On Skate's small Z80 target, that expressive freedom comes with a job for the runtime: recovering the storage when those structures are no longer reachable.
