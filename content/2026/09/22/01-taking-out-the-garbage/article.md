...
The garbage collector begins by tracing from its roots. Roots are known places where the running program can hold references, for example global variables and live values in active procedure calls. The collector follows references from every root through all the data structures that they lead to. Any allocated object that cannot be reached then becomes available for collection.
...
I'm starting with the most straightforward approach I can with regard to memory management. Keeping the collector small enough to fit the machine enables me to experiment with ways to reduce collection time or improve memory use. This kind of experimentation requires measurement and an assessment of the costs as well as the benefits. As I gain more experience with running Skate on a small system, I expect to gain insights into what works better and refine the design from there.
