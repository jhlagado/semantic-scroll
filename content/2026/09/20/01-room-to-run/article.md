...
We can judge those tradeoffs by the capacity we leave for the programmer: how much data a program can retain and how efficiently it can work with it. A smaller runtime earns its saving only if we preserve enough of Scheme to make useful programs practical. That is the test of this minimum implementation, and the reason to measure memory management as part of the language's cost.
