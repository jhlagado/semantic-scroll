...
Both groupings come from the same frontmatter table. Each article contributes a row with its date, tags, and series. The build applies different sorts to that same data. No extra structure is needed in the files themselves. What changes is how the list is constructed.
