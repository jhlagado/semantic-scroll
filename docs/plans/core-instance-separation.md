...
If separation becomes a priority, the engine can move to a dedicated repository such as `semantic-scroll-engine`. The instance repository would carry only `content/`, instance assets, templates, and site configuration. The engine repo would be pulled in using a simple upstream mechanism such as `git subtree` or a scripted vendor copy.
