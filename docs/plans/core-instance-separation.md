...
If a stronger dependency graph becomes useful, we can switch to npm workspaces inside a single repository. That would create `packages/engine` and `packages/prose-lint` with a `sites/example` instance. This is still one repo and uses npm only, but it adds multiple package manifests and a more explicit dependency graph.
