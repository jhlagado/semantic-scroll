...
Once you’ve got all that figured out, the AI will generate a **folder-based output**. Each post will live in its own date-based folder (like `/content/YYYY/MM/DD/NN-slug/`, with `/example/` used only as the bundled reference in this repo) and include a single markdown file plus an optional `assets/` subfolder for images, code, or media. The markdown file will have embedded frontmatter metadata for status, tags, title, optional summary, and optional thumbnail so index views are consistent while full article titles remain authored in the body and dates are rendered in the article metadata header.
